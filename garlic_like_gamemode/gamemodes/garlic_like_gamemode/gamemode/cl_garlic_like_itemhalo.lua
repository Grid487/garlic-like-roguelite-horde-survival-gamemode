...
hook.Add( "PostDrawTranslucentRenderables", "ItemHalo", function()
	...
	local ihl_lod_max = GetConVarNumber( "ihl_lod_max" )
	...
	for k,v in pairs ( ents.FindInSphere( LocalPlayer():GetPos(), ihl_lod_max ) ) do
		...
		if ( v:GetOwner() == NULL && v:GetNWBool( "MobDrop" ) == false ) and 
		( !table.HasValue( panicents, v:GetClass() ) ) and
		( ( v:IsWeapon() ) or ( table.HasValue( items_supplies, v:GetClass() ) ) or ( table.HasValue( items_ammo, v:GetClass() ) ) or ( v.PrintName ~= nil && ( v.Spawnable == true || v.AdminSpawnable == true ) ) ) then
...
--			} )
			...
			if ( table.HasValue( items_supplies, v:GetClass() ) ) then
				laser_color = ihl_color_medical
			end
			
			if ( table.HasValue( items_ammo, v:GetClass() ) ) then
				laser_color = ihl_color_ammo
			end
			
			if ( v:IsWeapon() && !v:IsScripted() ) then
				laser_color = ihl_color_wep
			end
			
			if ( v:IsWeapon() && v:IsScripted() ) then
				laser_color = ihl_color_swep
			end
			
			if ( v:GetClass() ~= "gmod_tool" ) then
				if ( distance <= ihl_lod_max ) then
					render.SetMaterial( mat_flare )
					render.DrawSprite( light_start, 12, 12, Color( laser_color.r, laser_color.g, laser_color.b, alpha3 ) )
				end
	...
	--			if ( LocalPlayer():GetEyeTrace().Entity == v ) and ( distance <= 512 ) then
	--				render.SetMaterial( mat_flare )
	--				render.DrawSprite( light_start, 32, 32, Color( laser_color.r, laser_color.g, laser_color.b, laser_color.a / 3 ) )
	--			end
					...
					if ( distance <= 256 ) and ( ihl_vis_shownames == 1 ) then
						cam.Start2D( light_end, Angle( 0, 0, 0 ), 0.25 )
							...
							if ( ihl_vis_shownames == 1 ) then
								if ( v:IsWeapon() ) then
									draw.WordBox( 6, text_pos.x, text_pos.y, v:GetPrintName(), "DermaDefaultBold", Color( 50, 60, 70, alpha ), textcolor )
								elseif ( v.PrintName ~= nil || v.PrintName ~= "" ) and ( !table.HasValue( items_supplies, v:GetClass() ) && !table.HasValue( items_ammo, v:GetClass() ) ) then
									draw.WordBox( 6, text_pos.x, text_pos.y, v.PrintName, "DermaDefaultBold", Color( 50, 60, 70, alpha ), textcolor )
								end
							end
						cam.End2D()
					end
